...
@echo off&title WSU&setlocal enabledelayedexpansion&(for /f "tokens=4 delims=. " %%a in ('ver') do if %%a neq 10 echo Sorry, WSU is incompatible with your Windows version. Please use Windows 10/11 to continue.&pause>nul&exit)&echo Please wait...&set "esc="&sc stop beep>nul 2>nul&set specsloaded=0
:prompt
cls&echo Windows Scripts Utility&echo.&echo    1. Chris Titus Tech^'s Windows Utility&echo    2. Microsoft Activation Scripts&echo    3. Windows10Debloater&echo    4. Windows11Debloat&echo    %esc%[92mS%esc%[0m. System specifications&echo    %esc%[92mB%esc%[0m. Battery report&echo    %esc%[92m0%esc%[0m. Quit&echo.&echo %esc%[93mChoose an option using your keyboard...%esc%[0m&choice /c 1234SB0 /n&cls&goto optn!errorlevel!
:optn1
set link=https://github.com/ChrisTitusTech/winutil&set command=start /wait powershell -Command "irm https://christitus.com/win|iex'"&goto exec
:optn2
...
:optn5
cls&(if %specsloaded%==0 (echo ^> Retrieving info, this may take up to a minute...&(for /f "tokens=*" %%a in ('powershell -c "(get-ciminstance -classname win32_operatingsystem).caption"') do set os=%%a)&(if not defined os set os=%esc%[91mN/A%esc%[0m)&(for /f "tokens=*" %%a in ('powershell -c "(get-ciminstance -classname win32_operatingsystem).osarchitecture"') do set arch=%%a)&(if not defined arch set arch=%esc%[91mN/A%esc%[0m)&(for /f "tokens=*" %%a in ('powershell -c "(get-ciminstance -classname win32_processor).name"') do set cpu=%%a)&(if not defined cpu set cpu=%esc%[91mN/A%esc%[0m)&(for /f "tokens=*" %%a in ('powershell -c "(get-ciminstance -classname win32_processor).numberofcores"') do set cores=%%a)&(if not defined cores set cores=%esc%[91mN/A%esc%[0m)&(for /f "tokens=*" %%a in ('powershell -c "(get-ciminstance -classname win32_processor).threadcount"') do set threads=%%a)&(if not defined threads set threads=%esc%[91mN/A%esc%[0m)&(for /f "tokens=*" %%a in ('powershell -c "(get-ciminstance -classname win32_videocontroller).name"') do set gpu=%%a)&(if not defined gpu set gpu=%esc%[91mN/A%esc%[0m)&(for /f "tokens=*" %%a in ('powershell -c "(Get-CimInstance -ClassName Win32_ComputerSystem).TotalPhysicalMemory / 1GB"') do set mem=%%a)&(if not defined mem set mem=%esc%[91mN/A%esc%[0m)&set specsloaded=1))&cls&echo OS       : !arch! !os!&echo Processor: !cpu! (!cores!C/!threads!T)&echo Graphics : !gpu!&echo Memory   : !mem! GB&echo.&echo %esc%[93mPress any key to go back...%esc%[0m&pause>nul&goto prompt
:optn6
echo ^> Generating battery report...&cd "c:\users\%username%\appdata\local\temp\"&powercfg /batteryreport /output "c:\users\%username%\appdata\local\temp\report.html">nul 2>nul&(if not exist report.html (echo ^> Failed to generate a report.&echo.&echo %esc%[93mPress any key to go back...%esc%[0m&pause>nul&goto prompt))&echo ^> Successfully generated a report.&echo.&echo %esc%[93mPress 1 to open the report file, 0 to ignore...%esc%[0m&choice /c 10 /n&goto optn6!errorlevel!
:optn61
start /wait report.html&echo %esc%[93mPress any key to go back...%esc%[0m&pause>nul
:optn62
del report.html&goto prompt
:optn7
exit
:connection
